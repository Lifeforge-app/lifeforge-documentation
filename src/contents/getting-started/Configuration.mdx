...
Next, head to the **Import collections** tab under the **Sync** section in the sidebar of the settings page. There, you'll find a huge textbox prompting you to paste the collection configurations. You can find the collection configurations for Lifeforge in the <code>lifeforge-api/pb_schema</code> directory. Copy all the content of the one and only <code>pb_schema.json</code> file and paste it into the textbox.

[![Pocketbase Import Collections](/assets/configurations/pocketbase-import-collections.png)](/assets/configurations/pocketbase-import-collections.png)

Once pasted, if everything goes well, you should be greeted with a whole list of changes that are automatically detected by the database. Scroll to the bottommost part of the page and click on the **Review** button. There will be a screen popping up showing all the diffs between the current database and the one that you're about to import.

[![Pocketbase Review Changes](/assets/configurations/pocketbase-diff.png)](/assets/configurations/pocketbase-diff.png)

It might look overwhelming, but since don't have anything in the database yet, there is nothing to worry about. Click on the **Confirm and Import** button to import the collections into the database.

And that's it! You have successfully imported all the collections required for Lifeforge into your Pocketbase database. 

### Create User

But, the database setup isn't done yet, we have something else to do. Now, head to the collection page by clicking the topmost icon on the leftmost sidebar. You should see a list of collections on the left side fo your screen.
...
Next, navigate to the <code>lifeforge-api</code> directory that you cloned earlier. You'll see a file named <code>.env.example</code> in the root directory. Copy this file and rename it to <code>.env.local</code>. Open it in your favourite text editor and fill in the following fields:

| Field           | Value                                                                                                         |
|-----------------|---------------------------------------------------------------------------------------------------------------|
| **MASTER_KEY**  | A random string that you can generate using a password generator. This key is used to encrypt sensitive data. |
| **PB_HOST**     | The URL to your Pocketbase server                                                                             |
...
And that's it! You have successfully configured your Lifeforge API server. Now, let's move on to the frontend configuration.
</section>

<section id="frontend-configuration">
## Frontend Configuration

This is the last step towards setting up your Lifeforge system. Navigate to the <code>lifeforge</code> directory that you cloned earlier. You will find a <code>.env.example</code> file in the root directory. Copy this file and rename it to <code>.env.local</code>. There is only two field that you need to fill in: 

| Field           | Value                                                                                                         |
|-----------------|---------------------------------------------------------------------------------------------------------------|
| **VITE_API_HOST**  | The URL to your Lifeforge API server. This should be the URL that your API server is running on. |
| **VITE_GOOGLE_API_KEY**     | The Google API key generate from your Google Cloud console. This key is used to enable the Google Maps and Google Fonts selector in the system. |
